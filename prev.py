...
# Map from table to builder function responsible for generating
# its entries pages. These functions will get as parameter the kv dict
# of values generated by 'fetch_values'. These will mostly just be the values
# of that entry's cols on the database, accessed by column name.
TABLE_TO_BUILDER = {
    'authors': author_page,
    'weblog': entry_page,
...
def fetch_values(db, table):
    """
    For each entry in the tables, fetches and constructs the kv dict
    of values to be passed to the builder function for generating its page.
    Returng a dict from slug to dict of key-values' for that page.
    """
...
def generate_section(db, table, builder):
    ...
    index_html = table_index_page(table, index_content_html)
...
def generate_all(db):
    ...
    db_blocks = [
        ...
        for slug in ['code', 'words']
    ]
    db_blocks_html = " ".join([row['html'] for row in db_blocks])
    write_file(INDEX_PATH, homepage(db_blocks_html))
    ...
    for table, builder in TABLE_TO_BUILDER.items():
        ...
        generate_section(db, table, builder)
